...
function hsgxia2shxurxu(nstr,nwbk)
{
	...
	<body>
			<form action="addMobanxinxi.do" name="form1" method="post">
				      <table width="100%" border="1" align="center" cellpadding="3" cellspacing="1"  style="border-collapse:collapse" bgcolor="#F2FDFF">
						<tr bgcolor="#E7E7E7">
							<td height="20" colspan="2" background="images/table_header.gif">添加模板信息</td>
						</tr>
...
<script language=javascript >  
function hsgcheck() {
		var mobanbianhao = $("#mobanbianhao").val();
		if(mobanbianhao==""||(mobanbianhao.length<6||mobanbianhao.length>12)){
			 $("#clabelmobanbianhao").html("<font color=red>模板编号不能为空且长度在3～12位之间！</font>");
			 $("input[id=mobanbianhao]").focus();
			 $("#querenzhuce").attr("disabled","disabled");
			 return false;
		} 
		else
		{
			$("#clabelmobanbianhao").html("");
			$.ajax({
				url : "quchongMobanxinxi.do",
				type : "post",
				data : "mobanbianhao=" + mobanbianhao,
				dataType : "json",
				success:function(result){
				if(result.info=="ng"){
					$("#clabelmobanbianhao").html("<font color=red>模板编号已存在，请更换！</font>");
					$("input[id=mobanbianhao]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
				...
				error:function(){
					$("#clabelmobanbianhao").html("系统异常，请检查错误！");
					$("input[id=mobanbianhao]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
